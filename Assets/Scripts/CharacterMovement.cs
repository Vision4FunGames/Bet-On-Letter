...
namespace Lean.Common
{
    public class CharacterMovement : MonoBehaviour
    {
        ...
        private void UpdatePosition(float factor)
        {
            ...
            print((remainingDelta - newDelta) + " factor " + factor + " remainingDelta: " + remainingDelta);

            if ((finalTransform.position.x > -2) && (finalTransform.position.x < 1.33f))
            {
                finalTransform.position += remainingDelta - newDelta;
                movementDirection = remainingDelta - newDelta;
                remainingDelta = newDelta;
            }
            if ((remainingDelta - newDelta).x > 0 && finalTransform.position.x <= -2)
            {
                finalTransform.position += remainingDelta - newDelta;
                movementDirection = remainingDelta - newDelta;
                remainingDelta = newDelta;
            }
            if ((remainingDelta - newDelta).x < 0 && finalTransform.position.x >= 1.33f)
            {
                finalTransform.position += remainingDelta - newDelta;
                movementDirection = remainingDelta - newDelta;
                remainingDelta = newDelta;
			...
			print("movementDirection "+movementDirection+ "factor "+factor + "remainingDelta: "+ remainingDelta+"dir: "+ (remainingDelta - newDelta));
			*/
        }
    }
}
